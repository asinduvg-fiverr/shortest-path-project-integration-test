...
void processShipment(struct Package* shipment, struct Map* map, struct Truck trucker[], struct Storage* storage) {
    // Check if weight is valid
    int x= 0;
    int y = 0;
    if (!readWeight(shipment->weight)) {x++;}
    if (!readSize(shipment->size)) {x++;}
    if (!readDestination(shipment->row, toupper(shipment->column)) || toupper(shipment->column) > 'Y' || shipment->row > MAP_ROWS) {x++;}
    ...
    if (!readWeight(shipment->weight)) {
        printf("weight (must be 1-1000 Kg.)");
        x--;
        y++;
        return;
    }
    ...
    struct Point dest = { destination.row, destination.col };
    struct Route directRoute = shortestPath((const struct Map*)&map, origin, dest); //This is not working; it's just returning 0,0 for points for some reason
    int pass = 0;
...
//        if (directRoute.points[x].col == blueRoute.points);
   // }
    ...
    //Choosing truck based on whats avaliable/size;
    int b = 1;  //1 means it has room;  0 means it doens't have room; b = blue, g = green, y = yellow
    int g = 1;
    y = 1;  //trucker[0] is green, trucker[1] is blue, trucker[2] is yellow

    //checks if shipment has the space or weight to fit in to truck[0]/green truck; if it doesn't then sets truck to 0
    //will know which ones have room at the end of it so it can be decided based on the closest of truckswhich has room
    if ((trucker[0].size + shipment->size > trucker[0].max_size) || trucker[0].weight + shipment->weight > trucker[0].max_weight) {
        b = 0;  
    }
    if ((trucker[1].size + shipment->size > trucker[1].max_size) || trucker[1].weight + shipment->weight > trucker[1].max_weight) {
        g = 0;
    }
    if ((trucker[2].size + shipment->size > trucker[2].max_size) || trucker[2].weight + shipment->weight > trucker[2].max_weight) {
        y = 0;
    }


    if(b + y + g == 0){}    //if none of them are open so it skips the if statemnets
    else if ((b + y + g) == 3) {   //if they're all open
    ...
    if (b == 0 && g == 0 && y == 0) {//if they're all out of room/weight then they can be added to storage
        storage->storage[storage->numPackage].column = shipment->column;
        storage->storage[storage->numPackage].size = shipment->size;
        storage->storage[storage->numPackage].row = shipment->row;
        storage->storage[storage->numPackage].column = shipment->column;
        storage->numPackage++;
    }
    //can add an else statement which will decide the route
    else {  //else statement added (delete if it's causing a problem)

        //Added the points from direct route to which route is most efficient; Haven't figured how to add the route message.
        if (b == 1) {    //Added a new check to see if the truck is full or not
            routeColor = "BLUE";

            trucker[1].size += shipment->size;  //Add the shipment into the truck
            trucker[1].weight += shipment->weight;
            ...
            trucker[0].size += shipment->size;  //Add the shipment into the truck
            trucker[0].weight += shipment->weight;
            ...
            trucker[2].size += shipment->size;  //Add the shipment into the truck
            trucker[2].weight += shipment->weight;
