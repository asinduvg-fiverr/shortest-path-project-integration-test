...
void processShipment(struct Package* shipment, struct Map map, struct Route* selectedRoute, const char* routeColor) {
    ...
    if (strcmp(divertMessage, "divert:") == 0) {
        ...
        struct Route directRoute = shortestPath((const struct Map*)&map,startPoint , destination); //WOrking now


        for (int z = 0; z < directRoute.numPoints; z++) {   //Show divert:
            P = 0;
            printf("Move %d:(%d, %c) \n",z+1, directRoute.points[z].row, directRoute.points[z].col + 'A');
            for (int y = 0; y < selectedRoute->numPoints; y++) {
                if (selectedRoute->points[y].col == directRoute.points[z].col && selectedRoute->points[y].row == directRoute.points[z].row) { P = 1; }
            }
            if (P == 0) {
                addPointToRoute(selectedRoute, directRoute.points[z].row, directRoute.points[z].col);
            }
        }
